...
const Missings = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/api/report`, formData, {
        headers: {
      ...
      if (response.status === 201) {
        Alert.alert('Success', 'The missing pet report has been submitted.');
        setPetName('');
        setPetType('');
        setDescription('');
        setContactInfo('');
        ...
        const petsResponse = await axios.get(`${API_BASE_URL}/api/pets`);
        setPublishedPets(petsResponse.data);
      } else {
        Alert.alert('Error', 'Your report could not be submitted. Please try again.');
      }
    } catch (error) {
      console.error(error);
    ...
    >
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          >
            <Image
              source={require('../assets/images/noun-lost-and-found-6632762 2.png')}
